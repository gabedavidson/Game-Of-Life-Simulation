...
void ProgramManager::doInitialPrompts(){
  ...
  // not an pertinent comment but the entire reason my 'algorithm' wasn't working
  //  correctly for a week was because I was missing this one line
  nextGenGrid->passGrid(activeGenGrid);
...
void ProgramManager::getGridFromFile(string inFile){
  ...
  getline(reader, t); // get width (columns)
  h = stoi(t);
  getline(reader, t); // get height (rows)
  w = stoi(t);
  ...
  while (!reader.eof()){
    ...
    for (int i = 0; i < t.length(); ++i){
      if (t.at(i) == 'X' || t.at(i) == 'x'){
        activeGenGrid->editGrid(heightIndex, i, true); // if 'x' or 'X', change inactive cell to active at coordinates
      }
    }
    ++heightIndex;
  }
...
void ProgramManager::run(){
  ...
  pastGenGrid = new GridOfLife(activeGenGrid->getWidth(), activeGenGrid->getHeight(), -1);
  pastGenGrid->passGrid(activeGenGrid);
  ...
  while (inProgress){
      ...
      for (int j = 0; j < activeGenGrid->getHeight(); ++j){
        ...
        if (numNeighbors < 2){
          if (activeGenGrid->cellIsOccupied(i, j)){
            nextGenGrid->editGrid(i, j, false); // change active to inactive
          }
        }

        // if neighbors == 2 -> cell remains stable

        // if neighbors == 3 -> if inactive, becomes active.
        if (numNeighbors == 3){
          if (!activeGenGrid->cellIsOccupied(i, j)){
            nextGenGrid->editGrid(i, j, true); // change inactive to active
          }
        }
    ...
    // if nextGenGrid equals pastGenGrid -> simulation is oscillating between two cycles apart
    else if (pastGenGrid->checkEquality(nextGenGrid)){
      inProgress = false;
      oscillationDetected = true;
    }
    // if not stable or empty
    else {
      pastGenGrid->passGrid(activeGenGrid); // copy activeGen grid onto pastGen grid
      activeGenGrid->passGrid(nextGenGrid); // copy nextGen grid onto activeGen grid
      ++numGeneration;
  ...
  // outro and end
  if (nextGenGrid->gridIsEmpty()){
    cout << "\nEnvironment is empty" << endl;
  }
  else if (oscillationDetected){
    cout << "\nInfinite oscillation detected after generation " << numGeneration << endl;
  }
  else {
    cout << "\nEnvironment is stable" << endl;
  }
  if (!pauseBetweenGens && !enterBetweenGens){
    cout << "\nSimulation ended on generation " << numGeneration << " as:" << endl;
    activeGenGrid->printGrid();
